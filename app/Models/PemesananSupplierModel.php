...
class PemesananSupplierModel extends Model
{
    ...
    public function insertData(){
        ...
        //pencatatan jurnal pada saat daftar renovasi (piutang pada pendapatan diterima dimuka)
                ...
                SELECT a.id_pembayaran as id_transaksi, b.kode_akun, a.tgl_bayar, b.posisi, ".$total_harga." as besar_bayar,b.kelompok,b.transaksi
                FROM pembayaran a
                CROSS JOIN transaksi_coa b
                WHERE a.id_pembayaran = ? AND b.transaksi = 'pembebanan' AND b.kelompok = 6
        ";
        ...
        $sql = "    INSERT INTO jurnal(`id_transaksi`, `kode_akun`, `tgl_jurnal`, `posisi_d_c`, `nominal`, `kelompok`, `transaksi`)
                ...
                FROM pembayaran a
                CROSS JOIN transaksi_coa b
                WHERE a.id_pembayaran = ? AND b.transaksi = 'pembebanan' AND b.kelompok = 7
        ";
        $dbResult = $this->db->query($sql, array($id_transaksi));
