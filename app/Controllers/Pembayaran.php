...
class Pembayaran extends BaseController
{
    ...
    public function index()
	{
        ...
        for($i=0;$i<count($ar);$i++){
            $ar[$i][1] = $this->materialmodel->getInfoMaterial($ar[$i][0], 'all');
            $ar[$i][2] = $this->materialmodel->getInfoMaterial($ar[$i][0], 'Isi');
            $ar[$i][3] = $this->materialmodel->getInfoMaterial($ar[$i][0], 'Kosong');
        }
        ...
        echo view('Pembayaran/Daftarmaterial', $data);
	}

    //listkamar
    public function listmaterial($id, $kos){
        //tambahkan pengecekan login
        if(!isset($_SESSION['nama'])){
            return redirect()->to(base_url('home')); 
        }
        ...
        //print_r($hasil);
        $data['idmaterial'] = $id;
        $data['namamaterial'] = $kos;
        echo view('HeaderBootstrap');
        echo view('SidebarBootstrap');
        echo view('Pembayaran/ListMaterial', $data);
    }
    ...
    public function listpembayaran($id_kamar){
        ...
        $data['pembayaran'] = $this->PembayaranModel->getHistoryPembayaranByIdKamar($id_material);
    ...
    public function inputpembayaran($id_pesan,$nokuitansi,$nama_material){
        helper('rupiah');
        //tambahkan pengecekan login
        if(!isset($_SESSION['nama'])){
            return redirect()->to(base_url('home')); 
